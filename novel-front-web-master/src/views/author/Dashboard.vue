  ...
  <div class="dashboard-page">
    <div class="dashboard-shell">
      <aside class="dashboard-side">
        ...
        <ul class="nav-list">
          <li><router-link class="nav-item on" :to="{ name: 'authorDashboard' }">大屏总览</router-link></li>
          <li><router-link class="nav-item" :to="{ name: 'authorBookList' }">小说管理</router-link></li>
          <li><router-link class="nav-item" :to="{ name: 'authorBookUpload' }">上传小说</router-link></li>
          <li><router-link class="nav-item" :to="{ name: 'authorUploadRecords' }">上传记录</router-link></li>
          <li><router-link class="nav-item" :to="{ name: 'authorCommentManage' }">书评管理</router-link></li>
        </ul>
      </aside>
      <main class="dashboard-main">
        <section class="hero-card">
          <div>
            <span class="eyebrow">Author Overview</span>
            <h2>作家专区大屏</h2>
            <p>查看作品、章节、书评和审核进度，适合作为毕业设计展示页。</p>
          </div>
          <div class="hero-stats">
            ...
            <div class="mini-stat">
              <span>章节数</span>
              <strong>{{ dashboard.totalChapters }}</strong>
            </div>
          </div>
        </section>
        ...
        <section class="chart-grid">
          ...
          <div class="chart-card tone">
            <h3>创作状态</h3>
            <p>当前账号的待审作品与待审章节已纳入审核流程，页面风格采用大屏仪表盘，展示效果更直观。</p>
            <div class="ring">
...
import { getAuthorDashboard } from "@/api/dashboard";
  ...
  setup() {
    ...
    const load = async () => {
      try {
        ...
        ElMessage.warning("作家专区统计加载失败，已使用默认数据");
      }
    };
    ...
    const cards = computed(() => [
      { label: "作品总数", value: state.dashboard.totalBooks },
      { label: "章节总数", value: state.dashboard.totalChapters },
      { label: "书评总数", value: state.dashboard.totalComments },
      { label: "待审作品", value: state.dashboard.pendingBooks },
...
.dashboard-page {
  ...
  background: radial-gradient(circle at top right, rgba(33, 150, 243, 0.22), transparent 28%),
    linear-gradient(180deg, #07111f 0%, #0f1c34 42%, #f4f7fb 42%, #f4f7fb 100%);
}
